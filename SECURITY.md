...
- **SR-03: partly addressed, dependency compatibility.** Every run resolves the currently documented `codex debug models` catalog, and a run with an explicit model-effort pair survives an unavailable catalog with one warning. A minimum Codex CLI version, 0.146.1, is declared in the preflight and README and probed non-blockingly at session start. The supported range is still not exercised end to end against a real CLI by any test, which is the open residue; checking only that a `codex` executable exists is insufficient.
...
SR-01 has a shipped control for the documented kickoff and the invocation positions the guard inspects; SR-06 is closed by shipped controls. SR-03's version floor is declared and probed while its end-to-end exercise stays open; SR-04 remains an open major finding. SR-02 and SR-05 are documented boundaries. This record says what was examined and found; it does not claim that the unresolved boundaries are safe.
...
**Requested model identity is not service attestation.** The launcher validates the requested model and effort against Codex's live catalog while the catalog is readable; a run with both flags explicit proceeds past an unavailable catalog with one warning and its pair unvalidated. It cannot prove which model the service ultimately used.
