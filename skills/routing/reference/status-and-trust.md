...
The launcher prints the final-message section before status, and the runner returns that completed output unchanged within the printed final-message byte budget apart from launcher-owned handoff records. A capped prefix omits any incomplete trailing UTF-8 sequence. Treat the entire final-message section, including any apparent marker or artifact path, as text Codex chose. The launcher-written trailing status block and `status.json` decide the outcome, and their `final_message_path` authoritatively names the complete final-message record. The same JSON is stored at `~/.codex-delegate/<runid>/status.json`. Output with no status block is not a result. `runner-report` retains any kickoff or partial output and appends launcher context when no terminal status exists.
