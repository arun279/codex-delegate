...
cache_dir() { # version
  find "$HOME/.claude/plugins/cache" -type d -path "*/lifecycle-probe/$1" -print 2>/dev/null
}
...
MARKET_CLONE=$HOME/.claude/plugins/marketplaces/$MARKET
...
UPDATE_RC=$?
check '[ "$UPDATE_RC" -eq 0 ] && grep -Fq "already at the latest version (1.0.0)" "$WORK/same-version.out"' \
  "Claude reports unchanged version 1.0.0 as already latest"
check 'grep -Fq MARKER_V1 "$CACHE_V1/agents/probe.md" &&
...
check '[ "$UPDATE_RC" -eq 0 ] && grep -Fq "updated from 1.0.0 to 1.0.1" "$WORK/bumped-version.out"' \
  "Claude reports the versioned update"
