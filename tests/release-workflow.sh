...
case "$CI_SCENARIO" in
  ...
  missing)
    # gh/jq without an explicit empty-result guard renders the absent object as two tabs.
    if ! printf '%s\n' "$*" | grep -Fq '.[0] // empty'; then
      printf '\t\t\n'
    fi
    ;;
...
case "$TAG_SCENARIO" in
  ...
  missing-then-matching)
    ...
    ;;
  missing-then-mismatched)
    if [ -e "$TAG_CREATED" ]; then
      printf '%s\t%s\n' 3333333333333333333333333333333333333333 "$tag_ref"
    else
      exit 2
    fi
    ;;
...
if extract_step "Require successful CI" "$CI_STEP"; then
  ...
  check 'grep -Fq "observed no matching run" "$WORK/ci-missing.out"' \
    "a missing CI run has an actionable diagnostic"
...
if extract_step "Create and push plugin tag" "$TAG_STEP"; then
  ...
  : >"$CLAUDE_CALLS"
  rm -f -- "$TAG_CREATED"
  TAG_SCENARIO=missing-then-mismatched bash -e "$TAG_STEP" >"$WORK/tag-created-mismatch.out" 2>&1
  TAG_RC=$?
  check '[ "$TAG_RC" -ne 0 ] && [ "$(wc -l <"$GIT_CALLS")" -eq 2 ] &&
         grep -Fq "plugin tag --push ." "$CLAUDE_CALLS"' \
    "a newly-created tag on another commit blocks release"
...
check '! grep -Fq "claude plugin validate" "$WORKFLOW" &&
       ! grep -Fq "scripts/npm-pack-check.py" "$WORKFLOW" &&
       ! grep -Fq "scripts/lint-frontmatter.py" "$WORKFLOW" &&
       ! grep -Fq "python3 -m py_compile" "$WORKFLOW" &&
       ! grep -Fq "shellcheck --shell=sh" "$WORKFLOW"' \
  "release relies on successful CI instead of copying CI-owned checks"
