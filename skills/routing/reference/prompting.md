...
The runner makes one blocking Bash call. If it receives HUP, INT, or TERM, the launcher stops the private Codex process group, writes `STOPPED` status, and exits with 128 plus the signal number. Do not add a caller-side poller or background waiter.
